...
-- Los casos son mutuamente excluyentes (0, 1 y 2 o más elementos en l)

-- sacar1: elimina la primera ocurrencia de un elemento en una lista
sacar1 :: Eq a => a -> [a] -> [a]
sacar1 = \x l -> case l of 
	[] -> [] -- también podríamos haber puesto error "no se encuentra el elemento"
	w:ws -> case w == x of 
		True -> ws  
		False -> w : sacar1 x ws 
...
selectSort = \l -> case l of 
	[] -> []				  
	x:xs  ->   minL l : selectSort (sacar1 (minL l) l) 
...
-- Esta funcion termina porque: 
  ...
  -- L1 : (Vx::a,Vxs::[a]) elem (minL (x:xs)) (x:xs) = True
  -- L2 : (Vl::[a]) elem e l = True => length(sacar1 e l) < length l 						  
...
-- LLamada recursiva con zs contenida en x:y:zs							  
...
-- - L1 : (Vx,y::a,Vys::[a]) length (fst (split (x:y:ys))) < length (x:y:ys)
-- - L2 : (Vx,y::a,Vys::[a]) length (snd (split (x:y:ys))) < length (x:y:ys)
...
-- - L1: (Vx::a,Vxs::[a],Vp::a->Bool) length (filter p xs) <= length xs < length (x:xs)
